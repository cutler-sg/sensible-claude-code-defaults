...
- [ ] **Open VSX namespace** *(MC: optional for the first pre-release; the workflow skips Open VSX until `OVSX_PAT` exists)* via `ovsx create-namespace cutler-sg`, plus its GitHub-linked token.
...
- [x] Verify the `.vsix` one more time by hand: no `node_modules`, no source maps, the bundled manifest present, *(done 2026-09-11: 11 files, 76 KB, all four checks clean, no token-shaped literal in the bundle; placeholder `icon.svg` dropped in PR #17)* `extensionKind` absent.
