...
- [x] Ship `0.1.0` as `--pre-release` *(published 2026-09-11 via release run 34573668038; GitHub pre-release with VSIX + SHA256SUMS at releases/tag/v0.1.0)* and dogfood it with known users before promoting (§9). There is no human review queue; the automated scan puts it live in minutes, and client-side auto-update is held ~2 hours for non-trusted publishers.
